...
controllers = {
    ...
    "MPC2": mpc2.ModelPredictiveController,
}
...
class ControlNode:
    ...
    def setup_pub_sub(self):
        ...
        rospy.Subscriber("/controller/set_path", XYHVPath, self.cb_path, queue_size=1)

        rospy.Service("/controller/follow_path", FollowPath, self.cb_path)

        rospy.Subscriber(rospy.get_param("~pose_topic", "/sim_car_pose/pose"), PoseStamped, self.cb_pose, queue_size=10)

        self.rp_ctrls = rospy.Publisher(rospy.get_param("~ctrl_topic", default="/vesc/high_level/ackermann_cmd_mux/input/nav_0"),
                                        AckermannDriveStamped,
                                        queue_size=2)

        self.rp_cte = rospy.Publisher(rospy.get_param("~cte_viz_topic", default="/controller/cte"), Float32, queue_size=2)

        self.rp_waypoints = rospy.Publisher(rospy.get_param("~waypoint_viz_topic", default="/controller/path/waypoints"), Marker, queue_size=2)

        self.rp_waypoint = rospy.Publisher(rospy.get_param("~selected_waypoint_viz_topic", default="/controller/path/selected_waypoint"),
                                           PoseStamped,
                                           queue_size=2)

        self.rp_path_viz = rospy.Publisher(rospy.get_param("~poses_viz_topic", default="/controller/path/poses"), PoseArray, queue_size=2)
    ...
    def cb_pose(self, msg):
        self.inferred_pose = [msg.pose.position.x, msg.pose.position.y, utils.rosquaternion_to_angle(msg.pose.orientation)]
